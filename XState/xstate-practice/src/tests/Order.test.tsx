...
import Order,{ getServices } from "../components/Order";
...
const getTestMachine = ()=>createMachine({
      ...
      states: {
        ...
        ordered: {
          on: {
            CONTINUE_SHOPPING: {actions: ["orderCompleted"],target: "shopping",
            },
          },
        },
      },
...
const shoppingTest = {
  test: async ({ target: { getByText } }: TestCycleContext) => {
    await waitFor(() => expect(() => getByText("shopping")).not.toThrowError());
  },
};
const cartTest = {
  test: async ({ target: { getByText } }: TestCycleContext) => {
    await waitFor(() => expect(() => getByText("cart")).not.toThrowError());
  },
};
const orderFailedTest = {
  test: async ({ target: { getByText } }: TestCycleContext) => {
    await waitFor(() => expect(() => getByText("orderFailed")).not.toThrowError());
  },
};
const placingOrderTest = {
  test: async ({ target: { getByText } }: TestCycleContext) => {
    await waitFor(() =>expect(() => getByText("placingOrder")).not.toThrowError());
  },
};
const orderedTest = {
  test: async ({ target: { getByText } }: TestCycleContext) => {
    await waitFor(() => expect(() => getByText("ordered")).not.toThrowError());
  },
};
